function [ complexHologram ] = IGHologram( dimensionsXY, pMatrix, mMatrix, qMatrix, beamRadiusPercent )
...
for i=1:grid(1)
    for j=1:grid(2)
        E(:,:,i,j)=IGBeam(pMatrix(i,j), mMatrix(i,j), qMatrix(i,j), beamRadiusPercent(i,j), xx, yy); % OAM
